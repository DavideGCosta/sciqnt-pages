...
With the sheer volume of financial news out there, it’s practically impossible for any human to process it all. Luckily, we have some powerful tools to make machines understand text—enter Sentence Embeddings, or Sentence Vectorization. Below, you’ll find a guide on how to implement this and key considerations to keep in mind while doing so.

[![Open in GitHub](https://img.shields.io/badge/%7C%20-Open%20in%20GitHub-blue?logo=github)](https://github.com/DavideGCosta/sciqnt-pages/tree/main/static/notebooks/docs/strategies/01-financial-news/02-sentence-embeddings.ipynb)
[![Open in Colab](https://img.shields.io/badge/%7C-Open%20in%20Colab-orange?style=flat&logo=googlecolab)](https://colab.research.google.com/github/DavideGCosta/sciqnt-pages/blob/tree/main/static/notebooks/docs/strategies/01-financial-news/02-sentence-embeddings.ipynb)
[![Open in Codespaces](https://img.shields.io/badge/%7C-Open%20in%20Codespaces-darkblue?style=flat&logo=git-for-windows&logoColor=white)](https://github1s.com/DavideGCosta/sciqnt-pages/tree/main/static/notebooks/docs/strategies/01-financial-news/02-sentence-embeddings.ipynb)
...
plotlyFig.update_layout(
    ...
    title='3D Visualization News using Manual Embeddings',
    legend_title='Titles',
...
### 1. Cosine Similarity
Similar sentences should have embeddings that are close to each other. We can measure this similarity to see if the embeddings reflect the semantic relationships between sentences.
...
# Create the heatmap
plotlyFig = go.Figure(data=go.Heatmap(
    z=masked_sim_matrix,
    x=titles,
    y=titles,
    colorscale='Greens',
...
### 2. Dimensionality Reduction & Embeddings Visualization:

As we demonstrated earlier, visualizing these embeddings can provide insights. Similar embeddings should cluster together in charts, showing that they capture the underlying context effectively. This way, we can visually validate that the embeddings represent the nuances of the text.
...
for i, embedding in enumerate(reduced_embeddings):
    plotlyFig.add_trace(go.Scatter3d(
        x=[embedding[0]],
        y=[embedding[1]],
        z=[embedding[2]],
...
plotlyFig.update_layout(
    ...
    title='3D Visualization of News Using SBERT',
    showlegend=False,  # Remove the legend
    margin=dict(l=0, r=0, b=0, t=40),
    template='plotly_dark'
)
